...
let
  ...
  fetchedRepos = lib.mapAttrs (repoKey: repoData:
  ...
  allSkills = lib.foldl' (acc: repoKey:
    let
      ...
      mappedSkills = lib.mapAttrs' (skillKey: skillData:
  ...
  skillFilesForAgent = agentName: agentConf:
    let
      ...
      targetedSkills = lib.filterAttrs (skillName: skillConf:
        skillConf.agents == [] || lib.elem agentName skillConf.agents
      ) enabledSkills;
    in
    lib.mapAttrs' (skillName: skillConf:
      let
        ...
        finalSkillName = if skillConf.rename != null then skillConf.rename else skillName;
      in
  ...
  homeFiles = lib.foldl' (acc: agentName:
    let
  ...
  options.modules.agentic-skills = {
    ...
    skills = mkOption {
      ...
      type = types.submodule ({ ... }: {
        options = lib.mapAttrs (skillName: skillData: mkOption {
          default = {};
    ...
    agents = mkOption {
      ...
      default = {};
      type = types.submodule ({ ... }: {
        options = lib.mapAttrs (name: defaultPath: mkOption {
